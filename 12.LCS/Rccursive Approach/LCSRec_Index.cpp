...
string findLCS(string S1, string S2, int i, int j, int LCS_table[][100]) {
    ...
    while (i > 0 && j > 0) {
        if (S1[i - 1] == S2[j - 1]) {
            lcs = S1[i - 1] + lcs;
            steps.push_back("D (" + string(1, S1[i - 1]) + ") {Index: " + to_string(i - 1) + ", LCS: " + lcs + "}");
            i--;
            j--;
        } else if (LCS_table[i - 1][j] > LCS_table[i][j - 1]) {
            steps.push_back("Up {Index: " + to_string(i - 1) + ", LCS: " + lcs + "}");
            i--;
        } else {
            steps.push_back("L {Index: " + to_string(j - 1) + ", LCS: " + lcs + "}");
            j--;
        }
    }
