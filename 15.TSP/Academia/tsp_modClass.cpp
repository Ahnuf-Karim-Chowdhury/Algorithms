        ...
        while (!temp.empty() && !found) {
            ...
            for (int i = 0; i < N; ++i) {
                if (curr.reducedMat[curr.vertexNum][i] != INT_MAX) {
                    ...
                    child.cpVisitedMat(curr.visited);
                    child.cpPathMat(curr.pathNodes);

                    setInfinity(child.reducedMat, curr.vertexNum, child.vertexNum, child.visited);

                    child.totalCost = curr.totalCost + curr.reducedMat[curr.vertexNum][i] + 
                                                reducedCost(child.reducedMat);
