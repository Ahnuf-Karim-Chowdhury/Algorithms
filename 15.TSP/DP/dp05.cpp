...
public:
    TSP(int matrix[N][N]) {
        ...
        dp.resize(N, vector<int>(pow(2, N), INF));  // DP table to track the cost of visiting all sets
        parent.resize(N, vector<int>(pow(2, N), -1));  // Parent table for path reconstruction
    }

    // Solve TSP using a dynamic programming approach
    void solveTSP(int start) {
        dp[start][pow(2, start)] = 0;  // Start at the given node with only the start node visited

        // Iterate over all sets of visited nodes
        for (int mask = 1; mask < pow(2, N); ++mask) {
            for (int u = 0; u < N; ++u) {
                if (mask & int(pow(2, u))) {  // u is in the current set
                    for (int v = 0; v < N; ++v) {
                        if (!(mask & int(pow(2, v))) && adjMatrix[u][v] != INF) {  // v is not yet visited
                            int newMask = mask | int(pow(2, v));
        ...
        int minCost = INF;
        int lastNode = -1;
        int fullMask = pow(2, N) - 1;  // All nodes visited
        ...
        while (currNode != -1) {
            ...
            mask ^= int(pow(2, currNode));  // Remove the current node from the mask
            currNode = temp;
        }
